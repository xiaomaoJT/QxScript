...
async function XiaoMaoWaOu() {
  ...
  // AES-128-ECB 加密函数
  const aesEncryptECB = (text, key) => {
    const encrypted = $.crypto.AES.encrypt($.crypto.enc.Utf8.parse(text), key, {
  ...
  const encryptJsonWithEapiKey = (jsonObject) => {
    try {
      ...
      const encryptedData = aesEncryptECB(text, eapiKey);
