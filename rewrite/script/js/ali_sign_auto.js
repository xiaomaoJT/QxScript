...
const lk = new ToolKit(`阿里云盘签到`, `AliYunPanCheckIn`, {
...
lk.userAgent =
  "Mozilla/5.0 (iPhone; CPU iPhone OS 15_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 D/C501C6D2-FAF6-4DA8-B65B-7B8B392901EB";
  ...
  if (lk.isGetCookie(/\/v2\/account\/token/)) {
    ...
    try {
      ...
      if (refreshToken) {
        ...
        lk.appendNotifyInfo(
          "🎉成功获取阿里云盘refresh_token，可以关闭相应脚本"
        );
      } else {
        lk.execFail();
...
function getReward(day) {
  if (getGoneDay(0) == getLastDay()) {
    $notify(
      "🚨XiaoMao_阿里云盘",
      "",
      "今天是月底了喔，记得手动去阿里云盘领取签到福利，明天将全部失效❗️"
    );
    return new Promise((resolve, _reject) => {
      const t = "领取奖励";
      let url = {
      ...
      lk.post(url, (error, _response, data) => {
        try {
          if (error) {
            ...
            if (dataObj.success) {
              lk.appendNotifyInfo(
                `✓${t}(第${day}天)，${dataObj?.result?.notice}`
              );
            } else {
              lk.execFail();
...
function joinTeam(layer = 0) {
  return new Promise(async (resolve, _reject) => {
    ...
    const weekOfYear = Math.ceil(
      (Math.round((lk.now.valueOf() - firstDayOfYear.valueOf()) / 86400000) +
        (firstDayOfYear.getDay() + 1 - 1)) /
        7
    );
        ...
        if (error) {
          ...
          if (dataObj.success) {
            ...
            if (joinedTeam && joinTeamId) {
              lk.appendNotifyInfo(
                `🎉${t}成功\n${dataObj?.result?.period}：${
                  dataObj?.result?.joinCount
                }(${dataObj?.result[joinedTeam + "WinRate"]})`
              );
