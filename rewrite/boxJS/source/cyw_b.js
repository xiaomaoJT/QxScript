...
!(async () => {
  ...
  function getGoneDay(n=0,yearFlag=true){let myDate=new Date();myDate.setDate(myDate.getDate()-n);let month=myDate.getMonth()+1;let day=myDate.getDate();let result=""+(yearFlag?myDate.getFullYear():"")+"/"+(month<10?"0"+month:month)+"/"+(day<10?"0"+day:day);return result}function XiaoMaoFunction(){$XiaoMaoSvip=API("XiaoMao");if($XiaoMaoSvip.read("CaiYunSvipYear")&&$XiaoMaoSvip.read("CaiYunSvipMonth")&&$XiaoMaoSvip.read("CaiYunSvipDay")){SvipDate=new Date($XiaoMaoSvip.read("CaiYunSvipYear")+"/"+$XiaoMaoSvip.read("CaiYunSvipMonth")+"/"+$XiaoMaoSvip.read("CaiYunSvipDay")).getTime();if(!SvipDate){$XiaoMaoSvip.notify(appName,"","会员日期设置错误，请输入正确的日期范围!");XiaoMaoSvip=getGoneDay(-10)}else{XiaoMaoSvip=$XiaoMaoSvip.read("CaiYunSvipYear")+"/"+$XiaoMaoSvip.read("CaiYunSvipMonth")+"/"+$XiaoMaoSvip.read("CaiYunSvipDay")}}else{XiaoMaoSvip=getGoneDay(-10)}XiaoMaoEndTime=new Date(XiaoMaoSvip).getTime()/1000}if($response.body){let requestUrl=$request.url;if(/^https?:\/\/biz\.(caiyunapp|cyapi)\.(com|cn)\/(membership_rights|v2\/user)?/.test(requestUrl)){obj.result.is_vip=true;obj.result.vip_type="s";obj.result.svip_given=365;obj.result.wt.svip_given=365;obj.result.is_primary=false;obj.result.is_visitor=false;obj.result.is_xy_vip=true;obj.result.vip_expired_at=XiaoMaoEndTime;obj.result.xy_svip_expire=XiaoMaoEndTime;obj.result.xy_vip_expire=XiaoMaoEndTime;obj.result.wt.vip.expired_at=XiaoMaoEndTime;obj.result.svip_expired_at=XiaoMaoEndTime;obj.result.wt.vip.svip_expired_at=XiaoMaoEndTime}$done({body:JSON.stringify(obj)})}else{$done({})}