...
var $XiaoMaoSvip="";var appName=`XiaoMao-彩云天气Svip_Token`;var XiaoMaoToken="";!(async()=>{await XiaoMaoFunction()})().catch((err)=>{$XiaoMaoSvip.error(err);setTimeout(()=>{$XiaoMaoSvip.done()},3000)}).finally(()=>{console.log(appName+"设置成功");setTimeout(()=>{$XiaoMaoSvip.done()},5000)});function XiaoMaoFunction(){$XiaoMaoSvip=API("XiaoMao");let requestUrl=$request.url;if($XiaoMaoSvip.read("CaiYunToken")&&/^https:\/\/wrapper\.cyapi\.cn\/v1?/.test(requestUrl)){XiaoMaoToken=$XiaoMaoSvip.read("CaiYunToken");let XiaoMaoHeader=$request.headers;XiaoMaoHeader["device-token"]=XiaoMaoToken;$done({headers:XiaoMaoHeader});return}else{console.log(appName+"未填写或获取失败，请前往XiaoMaoBoxJs填入有效Token，https://raw.githubusercontent.com/xiaomaoJT/QxScript/main/rewrite/boxJS/XiaoMao.json")}$done({})}