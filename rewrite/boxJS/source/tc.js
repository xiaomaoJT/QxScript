//------ 请于此处替换抓包关键数据 -------
let names = "Tally Counter"; //用于ua检测及弹窗提示 - 抓包查看ua
let productName = "com.tally.counter.clicker.streak.app"; //软件bundle_id - 抓包获取数据
let productType = "com.tally.counter.clicker.streak.lifetime1"; // 软件订阅类型 - 抓包获取数据

let appVersion = "1"; // 软件版本
...
if (
  ...
  (ua ? $request.headers["User-Agent"].includes(names) : true)
) {
  let receipt = {
