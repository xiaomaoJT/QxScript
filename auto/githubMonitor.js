const lk = nobyda();
const token = !lk.getVal("lkGithubMonitorToken")
  ? "null"
  : lk.getVal("lkGithubMonitorToken");
const isEnableLog = !lk.getVal("lkGithubMonitorIsEnableLog")
  ? true
  : JSON.parse(lk.getVal("lkGithubMonitorIsEnableLog"));
if (token == "null") {
  lk.msg(`Github监控`, ``, `未获取到token❌`);
  lk.done();
}
const tgNotifyUrl = !lk.getVal("lkGithubMonitorTgNotifyUrl")
  ? "null"
  : lk.getVal("lkGithubMonitorTgNotifyUrl");
if (tgNotifyUrl == "null") {
...
let repositories;
try {
  repositories = !lk.getVal("lkGithubMonitorRepo")
    ? []
    : JSON.parse(lk.getVal("lkGithubMonitorRepo"));
} catch (e) {
    ...
    (response) => {
      ...
      for (let i in file_list) {
        if (file_list[i].path == paths[current_pos]) {
          ...
          isDir =
            paths[current_pos].match(
              /\.(js|py|cpp|c|cpp|html|css|jar|png|jpg|bmp|exe)/
            ) == null
              ? true
              : false;
          $.log(
...
Promise.all(repositories.map(async (item) => await checkUpdate(item))).finally(
  () => $.done()
);
